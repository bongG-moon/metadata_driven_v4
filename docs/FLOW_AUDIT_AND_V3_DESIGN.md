...
| Metadata Architect | domain/table/filter/recipe metadata 경계를 설계 | 자연어 saving flow 3종 유지 |
...
- saving flow가 자연어 입력에서 metadata item을 만드는 구조를 이미 갖고 있다.
...
- domain/table catalog/main flow filter saving flow가 자연어 입력을 MongoDB 저장 item으로 정규화한다.
...
v3는 `metadata_driven_v2`를 구조적 base로 삼는다. 이유는 실제 서비스에서는 사용자가 Langflow 연결을 볼 때 flow의 책임이 분리되어 있어야 하고, metadata QA/실제 데이터 분석/리포트/운영 진단/metadata saving을 한 canvas에 과도하게 몰아넣지 않는 편이 유지보수에 유리하기 때문이다.
...
- 자연어 metadata saving flow 3종과 split-flow connection guide는 유지했다.

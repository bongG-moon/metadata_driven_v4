...
| If-Else | 문자열 비교 기반 conditional router. equals/contains/regex 등을 지원한다. | 명시 route hint, developer dummy route, saving type 선택처럼 결정적 조건에 좋다. | 자연어 분석/등록 의도 분류에는 부족하다. | deterministic gate로 사용 |
...
| `domain_saving` | `domain_saving_flow` | `raw_text` | domain metadata 등록 | `response_type=metadata_authoring`, `metadata_type=domain` |
| `dummy_domain_saving` | `dummy_domain_saving_flow` | `raw_text` | 개발/스모크 테스트용 domain metadata 등록 | `response_type=metadata_authoring`, `metadata_type=domain` |
| `table_catalog_saving` | `table_catalog_saving_flow` | `raw_text` | table catalog 등록 | `response_type=metadata_authoring`, `metadata_type=table_catalog` |
| `dummy_table_catalog_saving` | `dummy_table_catalog_saving_flow` | `raw_text` | 개발/스모크 테스트용 table catalog 등록 | `response_type=metadata_authoring`, `metadata_type=table_catalog` |
| `main_flow_filter_saving` | `main_flow_filters_saving_flow` | `raw_text` | main filter 등록 | `response_type=metadata_authoring`, `metadata_type=main_flow_filter` |
| `dummy_main_flow_filter_saving` | `dummy_main_flow_filter_saving_flow` | `raw_text` | 개발/스모크 테스트용 main filter 등록 | `response_type=metadata_authoring`, `metadata_type=main_flow_filter` |
...
| metadata saving | `response_type=metadata_authoring`, `metadata_type`, `message`, `write_result`, `trace` |
...
| `dummy_domain_saving_flow/` | domain authoring과 같은 `metadata_authoring` 더미 응답 |
| `dummy_table_catalog_saving_flow/` | table catalog authoring과 같은 `metadata_authoring` 더미 응답 |
| `dummy_main_flow_filter_saving_flow/` | main flow filter authoring과 같은 `metadata_authoring` 더미 응답 |

더미 saving flow는 MongoDB에 저장하지 않고, `raw_text` 보존과 응답 계약만 검증한다.

## 10. Metadata Saving 포함 방식

metadata saving flow도 router route에 포함한다.

원칙:

- 사용자가 UI에서 `domain/table_catalog/main_flow_filter`를 명시 선택하면 route classifier를 타지 않는다.
- 단일 자연어 입력창에서 들어온 경우에만 route classifier가 saving type을 판단한다.
- `raw_text`는 절대 합치거나 재작성하지 않는다.
- `review_notes`, `dry_run`, `duplicate_action`은 별도 field로 전달한다.
- MongoDB 저장은 기존 saving flow writer만 수행한다.
...
| `dummy_domain_saving_flow/` | domain authoring 호환 더미 응답 생성 |
| `dummy_table_catalog_saving_flow/` | table catalog authoring 호환 더미 응답 생성 |
| `dummy_main_flow_filter_saving_flow/` | main flow filter authoring 호환 더미 응답 생성 |
...
현재 authoring 화면이 각 saving flow를 직접 호출하는 구조는 migration 기간 동안 유지할 수 있다. 최종 목표는 router에서도 saving route를 실행할 수 있게 하는 것이다.
...
metadata 등록 문장은 saving type별 route로 가야 하고, raw text가 변형되지 않아야 한다.
...
5. metadata saving 3종 route를 연결한다.

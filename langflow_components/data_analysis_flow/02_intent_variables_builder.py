...
def build_variables(payload_value: Any, metadata_candidates_value: Any = None) -> dict[str, Any]:
    ...
    return {
        ...
        "state_summary": json.dumps(payload.get("state", {}), ensure_ascii=False, indent=2),
...
class IntentVariablesBuilder(Component):
    ...
    inputs = [DataInput(name="payload", display_name="페이로드", required=True), DataInput(name="metadata_candidates", display_name="메타데이터 후보", required=False)]
    outputs = [
        Output(name="question", display_name="사용자 질문", method="build_question", types=["Message"], group_outputs=True),
        Output(name="state_summary", display_name="상태 요약 JSON", method="build_state_summary", types=["Message"], group_outputs=True),
    ...
    def build_question(self) -> Message:
        return Message(text=build_variables(getattr(self, "payload", None), getattr(self, "metadata_candidates", None))["question"])

    def build_state_summary(self) -> Message:
        return Message(text=build_variables(getattr(self, "payload", None), getattr(self, "metadata_candidates", None))["state_summary"])

    def build_metadata_candidates(self) -> Message:
        return Message(text=build_variables(getattr(self, "payload", None), getattr(self, "metadata_candidates", None))["metadata_candidates"])

    def build_output_schema(self) -> Message:
        return Message(text=build_variables(getattr(self, "payload", None), getattr(self, "metadata_candidates", None))["output_schema"])

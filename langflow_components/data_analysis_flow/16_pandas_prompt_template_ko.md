...
- `available_helpers`에는 여러 함수가 동시에 들어올 수 있다. 실제로 필요한 함수만 선택된 `selected_steps`의 `function_name`, `input_text`, `source_alias`에 맞춰 호출한다.
- `match_product_tokens` helper가 제공된 경우 제품 속성 token 매칭을 직접 column filter로 대체하지 말고 helper를 먼저 호출한 뒤 그 결과를 이후 분석에 사용한다.
- `sample_passthrough_helper`는 여러 특화 함수 전달 형식을 확인하기 위한 더미 helper다. 실제 분석에서는 metadata가 명시적으로 선택한 경우에만 호출한다.

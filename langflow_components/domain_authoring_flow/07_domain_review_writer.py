...
def review_and_write(payload_value: Any, review_response: Any = "", mongo_uri: str = "", mongo_database: str = "", collection_name: str = "") -> dict[str, Any]:
    ...
    dry_run = bool(payload.get("request", {}).get("dry_run", True))
    action = payload.get("duplicate_decision", {}).get("action", "ask")
    duplicate_blocked = bool(payload.get("existing_matches")) and action == "ask"
    ...
    if not ready:
        ...
        return next_payload
    if dry_run:
        next_payload["write_result"] = {"success": True, "ready_to_save": True, "dry_run": True, "saved_count": 0, "would_save_count": len(payload.get("items", [])), "message": "드라이런입니다. MongoDB에는 저장하지 않았습니다.", "keys": [_key(item) for item in payload.get("items", [])]}
        return next_payload
    write_result = _write_to_mongodb(payload, mongo_uri, mongo_database, collection_name)
...
def _write_to_mongodb(payload: dict[str, Any], mongo_uri: str, mongo_database: str, collection_name: str) -> dict[str, Any]:
    ...
    try:
        ...
        for item in payload.get("items", []):
            ...
            operations.append({"key": f"{doc.get('section')}:{doc.get('key')}", "operation": "upserted"})
...
def _deterministic_review(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    for item in payload.get("items", []):
        if not item.get("section") or item.get("section") not in ALLOWED_SECTIONS:
            errors.append({"type": "unsupported_section", "message": f"지원하지 않는 section입니다: {item.get('section')}"})
        if not item.get("key"):
            errors.append({"type": "missing_key", "message": "key가 없습니다."})
        if not item.get("payload"):
            errors.append({"type": "missing_payload", "message": "payload가 비어 있습니다."})
        if "source_config" in item.get("payload", {}) or "query_template" in item.get("payload", {}):
            errors.append({"type": "domain_source_config_forbidden", "message": "domain에는 source/query config를 저장하지 않습니다."})
    return {"ready_to_save": bool(payload.get("items")) and not errors, "item_reviews": [{"key": _key(item), "ready_to_save": not errors, "warnings": [], "errors": errors} for item in payload.get("items", [])], "errors": errors, "supplement_requests": []}
...
def _merge_review(llm_review: dict[str, Any], deterministic_review: dict[str, Any]) -> dict[str, Any]:
    ...
    deterministic_errors = deterministic_review.get("errors", [])
    ...
    merged["ready_to_save"] = bool(merged.get("ready_to_save")) and bool(deterministic_review.get("ready_to_save")) and not merged_errors and not merged_supplements

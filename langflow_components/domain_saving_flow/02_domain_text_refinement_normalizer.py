...
def normalize_refinement(payload_value: Any, llm_response: Any) -> dict[str, Any]:
    ...
    refinement = {
        "refined_text": str(parsed.get("refined_text") or raw_text),
        "needs_more_input": bool(parsed.get("needs_more_input", False)),
        "missing_information": _list(parsed.get("missing_information")),
        "assumptions": _list(parsed.get("assumptions")),
        "remaining_questions": _list(parsed.get("remaining_questions")),
    }
    next_payload = deepcopy(payload)
    next_payload["refinement"] = refinement
    if refinement["needs_more_input"]:
        next_payload.setdefault("warnings", []).append({"type": "needs_more_input", "message": "정제 단계에서 보충 정보가 필요합니다."})
    return next_payload
...
class DomainTextRefinementNormalizer(Component):
    display_name = "02 도메인 텍스트 정제 정규화기"

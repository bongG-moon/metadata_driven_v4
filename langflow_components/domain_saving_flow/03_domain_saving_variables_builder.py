...
def build_variables(payload_value: Any) -> dict[str, Any]:
    ...
    existing_summary = [{"section": item.get("section"), "key": item.get("key"), "payload_keys": sorted((item.get("payload") or {}).keys())} for item in existing if isinstance(item, dict)]
    return {
        "existing_metadata_summary": json.dumps(existing_summary[:50], ensure_ascii=False, indent=2),
        "refined_text": payload.get("refinement", {}).get("refined_text", ""),
...
class DomainSavingVariablesBuilder(Component):
    ...
    description = "Langflow 프롬프트 템플릿에 연결할 정제 텍스트와 기존 메타데이터 요약 변수를 제공합니다."

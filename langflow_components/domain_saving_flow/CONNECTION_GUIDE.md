# Domain Saving Flow Connection Guide
...
| --- | --- | --- |
| 1 | `Text Input.message` 또는 `Chat Input.message` | `00 Domain Saving Request Loader.raw_text` |
| 2 | `00 Domain Existing Items Loader.existing_items` | `00 Domain Saving Request Loader.existing_items` |
| 3 | `00 Domain Saving Request Loader.payload_out` | `01 Domain Text Refinement Variables Builder.payload` |
...
| 6 | `00 Domain Saving Request Loader.payload_out` | `02 Domain Text Refinement Normalizer.payload` |
| 7 | `Langflow Agent/LLM.output` | `02 Domain Text Refinement Normalizer.llm_response` |
| 8 | `02 Domain Text Refinement Normalizer.payload_out` | `03 Domain Saving Variables Builder.payload` |
| 9 | `03 Domain Saving Variables Builder.existing_metadata_summary` | `Langflow Prompt Template: 03_saving_prompt_template_ko.md.existing_metadata_summary` |
| 10 | `03 Domain Saving Variables Builder.refined_text` | `Langflow Prompt Template: 03_saving_prompt_template_ko.md.refined_text` |
| 11 | `Langflow Prompt Template: 03_saving_prompt_template_ko.md.output` | `Langflow Agent/LLM.input` |
| 12 | `02 Domain Text Refinement Normalizer.payload_out` | `04 Domain Saving Result Normalizer.payload` |
| 13 | `Langflow Agent/LLM.output` | `04 Domain Saving Result Normalizer.llm_response` |
| 14 | `04 Domain Saving Result Normalizer.payload_out` | `05 Domain Similarity Checker.payload` |
...
| 20 | `07 도메인 검수/저장 처리기.payload_out` | `08 도메인 등록 응답 정규화기.payload` |
| 21 | `08 도메인 등록 응답 정규화기.payload_out` | `09 도메인 등록 메시지 어댑터.payload` |
| 22 | `08 도메인 등록 응답 정규화기.payload_out` | `10 도메인 등록 API 응답 생성기.payload` |
| 23 | `09 도메인 등록 메시지 어댑터.message` | `10 도메인 등록 API 응답 생성기.display_message` |
| 24 | `09 도메인 등록 메시지 어댑터.message` | `Chat Output.message` |
...
| `10 도메인 등록 API 응답 생성기.api_response` | downstream Data Output | Web/API에서 구조화 응답을 직접 읽을 때 사용한다. |
| `10 도메인 등록 API 응답 생성기.api_message` | downstream Message Output | Run Flow가 Message output을 안정적으로 읽어야 할 때 사용한다. |

## Final Output Usage

- Playground에서는 `09 도메인 등록 메시지 어댑터.message -> Chat Output.message`를 사용한다.
- Web/API에서는 `10 도메인 등록 API 응답 생성기.api_response`를 우선 사용한다.
- JSON 문자열 형태가 더 안정적인 환경에서는 `10 도메인 등록 API 응답 생성기.api_message`를 함께 노출한다.

## Fixed/Manual Inputs

- `00 Domain Saving Request Loader.duplicate_action`: 기본은 `ask`. 중복 처리 재실행 때 `merge`, `replace`, `skip`, `create_new` 중 하나로 바꾼다.
- `00 Domain Saving Request Loader.dry_run`: 실제 저장 전에는 `true`, 검수 후 저장할 때만 `false`.

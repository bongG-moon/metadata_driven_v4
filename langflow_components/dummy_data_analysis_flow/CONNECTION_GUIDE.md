...
다만 Playground에서 실제 `data_analysis_flow` 응답과 비슷하게 보이도록 `answer_sections` 기반의 서비스형 답변, 결과 테이블, 적용 기준, 분석 과정 요약, 분석 근거를 포함한다.
...
제품 token 매칭이 필요 없는 더미 질문에서는 `분석 근거`의 helper 매칭 상세가 생략될 수 있다.

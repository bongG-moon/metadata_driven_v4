...
다만 Playground에서 실제 `data_analysis_flow` 응답과 비슷하게 보이도록 답변, 결과 테이블, 의도 분석, 데이터 조회, pandas 코드/실행 섹션을 모두 포함한다.
...
제품 token 매칭이 필요 없는 더미 질문에서는 `제품/조건 매핑 결과` 섹션만 생략될 수 있다.

...
def build_response(payload_value: Any) -> dict[str, Any]:
    ...
    return {
        ...
        "warnings": [{"type": "dummy_authoring_flow", "message": "실제 LLM/MongoDB 저장 없이 더미 등록 응답을 반환했습니다."}],
...
class DummyTableCatalogAuthoringResponseBuilder(Component):
    display_name = "01 더미 테이블 카탈로그 등록 응답 생성기"
    description = "테이블 카탈로그 등록 flow와 같은 형태의 더미 API 응답과 메시지를 생성합니다."

# Dummy Table Catalog Saving Flow 연결 가이드

실제 MongoDB 저장 없이 table catalog saving 응답 계약을 검증하는 개발용 flow다.
...
`01 더미 테이블 카탈로그 등록 응답 생성기.API 응답`은 실제 table catalog saving flow의 최종 응답과 같은 기본 형태를 가진다.

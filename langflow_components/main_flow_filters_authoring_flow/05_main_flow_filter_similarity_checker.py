...
def check_similarity(payload_value: Any, existing_items_value: Any = None) -> dict[str, Any]:
    ...
    existing_keys = {str(item.get("filter_key") or item.get("key") or "").lower() for item in existing if isinstance(item, dict)}
    matches = []
    for item in payload.get("items", []):
        key = str(item.get("filter_key") or "")
        if key.lower() in existing_keys:
            matches.append({"new_key": key, "existing_key": key, "match_type": "same_key", "recommended_action": "merge", "reason": "같은 filter_key가 이미 존재합니다."})
...
class MainFlowFilterSimilarityChecker(Component):
    display_name = "05 메인 플로우 필터 유사도 확인기"
    description = "메인 플로우 필터 저장 후보와 기존 항목의 같은 필터 키 충돌을 확인합니다."

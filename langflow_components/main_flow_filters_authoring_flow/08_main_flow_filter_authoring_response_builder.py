...
def build_response(payload_value: Any) -> dict[str, Any]:
    ...
    return {"response_type": "metadata_authoring", "metadata_type": "main_flow_filter", "success": bool(wr.get("success")), "message": wr.get("message") or "저장하지 않았습니다.", "write_result": wr, "trace": {"raw_text_preview": payload.get("trace", {}).get("raw_text_preview", ""), "generated_items_preview": payload.get("trace", {}).get("generated_items_preview", []), "existing_matches": payload.get("existing_matches", [])}}
...
class MainFlowFilterAuthoringResponseBuilder(Component):
    display_name = "08 메인 플로우 필터 등록 응답 생성기"
    description = "메인 플로우 필터 등록 최종 API 응답과 사용자 메시지를 만듭니다."

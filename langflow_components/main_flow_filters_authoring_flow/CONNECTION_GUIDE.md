# Main Flow Filters Authoring Flow Connection Guide
...
| --- | --- | --- |
| 1 | `Text Input.message` 또는 `Chat Input.message` | `00 Main Flow Filter Authoring Request Loader.raw_text` |
| 2 | `00 Main Flow Filter Existing Items Loader.existing_items` | `00 Main Flow Filter Authoring Request Loader.existing_items` |
| 3 | `00 Main Flow Filter Authoring Request Loader.payload_out` | `01 Main Flow Filter Text Refinement Variables Builder.payload` |
...
| 14 | `04 Main Flow Filter Authoring Result Normalizer.payload_out` | `05 Main Flow Filter Similarity Checker.payload` |
...
- `00 Main Flow Filter Authoring Request Loader.duplicate_action`: 기본은 `ask`. 중복 처리 재실행 때 `merge`, `replace`, `skip`, `create_new` 중 하나로 바꾼다.
- `00 Main Flow Filter Authoring Request Loader.dry_run`: 실제 저장 전에는 `true`, 검수 후 저장할 때만 `false`.

...
def review_and_write(payload_value: Any, review_response: Any = "", mongo_uri: str = "", mongo_database: str = "", collection_name: str = "") -> dict[str, Any]:
    ...
    if not ready:
        ...
        next_payload["write_result"] = {"success": True, "ready_to_save": True, "dry_run": True, "saved_count": 0, "would_save_count": len(payload.get("items", [])), "message": "드라이런입니다. MongoDB에는 저장하지 않았습니다.", "keys": [item.get("filter_key") for item in payload.get("items", [])]}
    else:
...
def _deterministic_errors(payload: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for item in payload.get("items", []):
        ...
        if not (p.get("display_name") or p.get("aliases")):
            errors.append({"type": "missing_label", "message": "display_name 또는 aliases가 필요합니다."})
        for field in ("operator", "value_type", "value_shape"):
            if not p.get(field):
                errors.append({"type": "missing_required_field", "message": f"{field}가 필요합니다.", "field": field})
    return errors
...
def _write_to_mongodb(payload: dict[str, Any], mongo_uri: str, mongo_database: str, collection_name: str) -> dict[str, Any]:
    ...
    try:
        ...
        for item in payload.get("items", []):
            ...
            operations.append({"key": doc.get("filter_key"), "operation": "upserted"})

...
def build_response(payload_value: Any) -> dict[str, Any]:
    ...
    return {
        ...
        "metadata_authoring": {
            ...
            "requires_duplicate_decision": bool(write_result.get("requires_duplicate_decision")),
...
def _summary(status: str, write_result: dict[str, Any], items: list[Any]) -> str:
    ...
    if status == "needs_decision":
        return "비슷한 기존 메인 플로우 필터가 있어 저장을 멈췄습니다. 중복 처리 방식을 선택해 주세요."
...
def _notices(write_result: dict[str, Any], review: dict[str, Any], payload: dict[str, Any]) -> list[dict[str, str]]:
    ...
    if write_result.get("requires_duplicate_decision"):
        notices.append({"type": "duplicate", "title": "중복 확인", "message": "replace, skip, ask 중 어떤 방식으로 처리할지 선택해야 합니다."})
    if payload.get("existing_matches"):
...
def _next_steps(status: str) -> list[str]:
    ...
    if status == "needs_decision":
        return ["중복 처리 방식을 선택한 뒤 다시 실행하세요."]
    if status == "needs_input":
...
def _compact_write_result(write_result: dict[str, Any]) -> dict[str, Any]:
    allowed = {"success", "ready_to_save", "dry_run", "saved_count", "would_save_count", "requires_duplicate_decision", "database", "collection_name", "message", "errors", "keys", "operation_by_key"}

...
def build_api_payload(api_response_value: Any) -> dict[str, Any]:
    ...
    if not payload:
        payload = {"response_type": "metadata_authoring", "metadata_type": "table_catalog", "status": "error", "message": "API 응답이 비어 있습니다."}
    payload.setdefault("response_type", "metadata_authoring")
    payload.setdefault("metadata_type", "table_catalog")
...
def _payload(value: Any) -> dict[str, Any]:
    ...
    if isinstance(text, str) and text.strip():
        try:
            ...
            return {"response_type": "metadata_authoring", "metadata_type": "table_catalog", "message": text}
        return deepcopy(parsed) if isinstance(parsed, dict) else {}
    return {}


class TableCatalogAuthoringApiAdapter(Component):
    display_name = "09 테이블 카탈로그 등록 API 연결 어댑터"
    description = "테이블 카탈로그 등록 결과를 Web/Run API에서 안정적으로 읽을 수 있는 JSON 메시지로 변환합니다."

"""Dry-run first metadata authoring runtime.

This module does not call an LLM and does not write to MongoDB by default.
It provides the deterministic parts around Korean prompt construction,
LLM JSON normalization, duplicate checks, review enforcement, and dry-run
write results.
"""
...
def build_authoring_payload(
    ...
    return {
        ...
        "existing_matches": [],
        "conflict_warnings": [],
        "duplicate_decision": {"action": duplicate_action, "target_key": ""},
        "review": {},
        "write_result": {},
        "trace": {
...
def build_text_refinement_prompt(metadata_type: str, raw_text: str) -> str:
    """Build Korean refinement prompt text."""

    return f"""너는 제조 AI agent의 metadata 등록 보조자다.

목표:
- 사용자가 쓴 자연어 원문을 저장 후보 JSON으로 바꾸기 쉬운 한국어 설명으로 정리한다.
- 원문에 없는 컬럼명, SQL, 조건, source id를 추측하지 않는다.
- 원문이 부족하면 저장하지 말고 `missing_information`에 무엇이 부족한지 적는다.

metadata_type: {metadata_type}

반환은 반드시 JSON object 하나로 한다.
...
def build_authoring_json_prompt(
    ...
    """Build Korean authoring JSON prompt text."""
    ...
    return f"""너는 제조 AI agent의 metadata authoring JSON 작성자다.
...
def build_review_prompt(metadata_type: str, items: list[dict[str, Any]], conflicts: list[dict[str, Any]]) -> str:
    """Build Korean review prompt text."""
    ...
    return f"""너는 제조 AI agent metadata 저장 검수자다.

목표:
- 저장해도 되는 item인지 검수한다.
- 실행 불가능하거나 필수 식별자가 없거나 secret/token/password가 있으면 저장 불가로 판단한다.
- 중복/유사 항목이 있으면 사용자 선택이 필요하다고 판단한다.
- 보충 요청은 한국어로 구체적으로 작성한다.
...
def normalize_text_refinement(payload: dict[str, Any], llm_response: str | dict[str, Any]) -> dict[str, Any]:
    data = llm_response if isinstance(llm_response, dict) else parse_llm_json(llm_response)
    refinement = {
        "refined_text": str(data.get("refined_text") or payload["request"].get("raw_text") or ""),
        "needs_more_input": bool(data.get("needs_more_input", False)),
        "missing_information": ensure_list(data.get("missing_information")),
        "assumptions": ensure_list(data.get("assumptions")),
        "remaining_questions": ensure_list(data.get("remaining_questions")),
    }
    payload = deepcopy(payload)
    payload["refinement"] = refinement
    if refinement["needs_more_input"]:
        payload["warnings"].append(
            make_error("needs_more_input", "정제 단계에서 보충 정보가 필요하다고 판단했습니다.")
        )
    return payload
    ...
    for item in items:
        ...
        if key and key.lower() in existing_by_key:
            ...
            warnings.append(
                {
                    "severity": "blocker",
                    "message": "같은 key가 있어 처리 방식 선택이 필요합니다.",
                    "new_item_key": key,
        ...
        for existing in existing_items:
            overlap = new_aliases.intersection(_aliases(existing))
            if overlap:
                matches.append(
                    {
                        "new_key": key,
...
def apply_duplicate_check(
    ...
    if matches and payload["duplicate_decision"]["action"] == "ask":
        payload["warnings"].append(
            make_error("duplicate_decision_required", "비슷하거나 같은 기존 metadata가 있어 저장 전 선택이 필요합니다.")
        )
    return payload
...
def build_deterministic_review(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    duplicate_needs_choice = bool(payload.get("existing_matches")) and payload["duplicate_decision"]["action"] == "ask"
    ready_to_save = bool(item_reviews) and not errors and not duplicate_needs_choice
...
def apply_review_and_write(
    ...
    if action == "skip":
        payload["write_result"] = {
            ...
            "ready_to_save": False,
            "saved_count": 0,
            "message": "저장하지 않았습니다. 사용자가 저장하지 않음을 선택했습니다.",
            "errors": [],
        }
        return payload
    ...
    if not review_result.get("ready_to_save"):
        payload["write_result"] = {
            ...
            "requires_duplicate_decision": bool(payload.get("existing_matches")) and action == "ask",
    ...
    if dry_run:
        payload["write_result"] = {
            ...
            "dry_run": True,
            "saved_count": 0,
            "would_save_count": len(payload.get("items", [])),
    ...
    for item in payload.get("items", []):
        key = metadata_item_key(payload["metadata_type"], item)
        existing = store.get_item(payload["metadata_type"], key)
        if existing and action == "merge":
            merged = deep_merge(existing, item)
            store.upsert_item(payload["metadata_type"], merged)
            operation = "merged"
        elif existing and action == "replace":
            store.upsert_item(payload["metadata_type"], item)
            operation = "replaced"
            ...
            store.upsert_item(payload["metadata_type"], item)
            operation = "created"
        operation_by_key.append({"key": key, "operation": operation})

    payload["write_result"] = {
        "success": not payload["errors"],
        "ready_to_save": not payload["errors"],
        "saved_count": len(operation_by_key),
...
def _not_saved_message(payload: dict[str, Any]) -> str:
    if payload.get("existing_matches") and payload["duplicate_decision"]["action"] == "ask":
        return "비슷한 기존 정보가 있어 아직 저장하지 않았습니다. 처리 방식을 선택해 주세요."
...
def build_authoring_response(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if write_result.get("requires_duplicate_decision"):
        response["duplicate_options"] = [
...
def _is_non_actionable_raw_block(block: str) -> bool:
    ...
    if text.startswith("## "):
        return True
    if text in {"아래 블록들은 Domain Authoring Flow에 하나씩 넣기 위한 기준 입력입니다.", "## 단일 항목 예시"}:
        return True
    return False
...
def run_authoring_dry_run(
    ...
    refinement = {
        ...
        "needs_more_input": False,
        "missing_information": [],
        "assumptions": ["드라이런에서는 원문을 변형하지 않고 정제문으로 전달했습니다."],
    }
    payload = normalize_text_refinement(payload, refinement)
    if authoring_response is None:
        payload["warnings"].append(
            make_error("llm_required", "저장 후보 item 생성을 위해 한국어 authoring prompt와 LLM 응답이 필요합니다.")
        )
        payload["prompts"] = {
            "text_refinement_prompt_ko": build_text_refinement_prompt(metadata_type, raw_text),
        ...
        payload["write_result"] = {
            ...
            "ready_to_save": False,
            "saved_count": 0,
            "message": "드라이런입니다. LLM authoring JSON 응답이 없어 저장 후보를 만들지 않았습니다.",
    ...
    payload["prompts"] = {
        "text_refinement_prompt_ko": build_text_refinement_prompt(metadata_type, raw_text),
        "authoring_json_prompt_ko": build_authoring_json_prompt(metadata_type, payload["refinement"]["refined_text"], existing_items),
        "review_prompt_ko": build_review_prompt(metadata_type, payload["items"], payload["conflict_warnings"]),
    }
    payload["api_response"] = build_authoring_response(payload)
    return payload

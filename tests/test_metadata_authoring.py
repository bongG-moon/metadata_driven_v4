...
def test_korean_prompts_are_korean_first():
    refinement_prompt = build_text_refinement_prompt("domain", "DA는 D/A1부터 D/A6까지야.")
    authoring_prompt = build_authoring_json_prompt("domain", "DA 공정 그룹 설명", [])

    assert "한국어" in refinement_prompt
    assert "사용자 원문" in refinement_prompt
...
def test_duplicate_ask_never_writes():
    existing = [
        {
    ...
    assert result["write_result"]["success"] is False
    assert result["write_result"]["requires_duplicate_decision"] is True
    assert "비슷한 기존 정보" in result["write_result"]["message"]

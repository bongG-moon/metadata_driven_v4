"""Generate dry-run metadata authoring reports from raw text files.
...
review what would be sent to the authoring flows.
"""
...
from reference_runtime.metadata_authoring import run_authoring_dry_run, split_raw_text_blocks, timestamp_utc, write_json_report
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Dry-run metadata authoring flow inputs without MongoDB writes.")
    parser.add_argument("--workspace", default=".", help="Workspace root containing txt metadata inputs.")
    parser.add_argument("--output-dir", default="", help="Report directory. Defaults to validation_runs/metadata_authoring_dry_run/<timestamp>.")
    ...
    output_dir = Path(args.output_dir).resolve() if args.output_dir else workspace / "validation_runs" / "metadata_authoring_dry_run" / timestamp_utc()
